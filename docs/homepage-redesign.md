...
| `public/assets/home-response-path.webp` | Original AI-generated East Texas woodland path illustration, used as a supporting graphic for the regional-response audience path. No people, buildings, logos, or documentary claims. |
...
### Response-Path Illustration Prompt

Use case: illustration-story. Create a polished vertical illustrated path graphic with more realistic East Texas trees, suitable to replace a simple icon-like tree path on the Memory Health Life Center homepage. Use the existing homepage illustration as a style reference only for line quality, color palette, and humane warmth. Show a narrow vertical slice of an East Texas woodland garden path, with a gently winding accessible limestone path moving downward through native plantings. Include realistic loblolly pines, mature oaks, understory shrubs, grasses, and small wildflowers along the path, with varied tree silhouettes and believable bark and foliage detail. Use refined hand-painted watercolor and gouache with fine pencil detail, warm daylight, forest teal, sage green, pine green, oak bark gray-brown, muted clay accents, pale sky wash, warm cream paper, and generous negative space. No text, logos, labels, people, buildings, medical symbols, generic flat icons, repeating clip-art trees, photorealism, dark heavy background, frame, or border.
